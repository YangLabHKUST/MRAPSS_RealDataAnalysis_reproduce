run_egger <- function(dat, Threshold, exposure="Trait 1", outcome="Trait 2"){
  res = NULL
  ...
  for(Thresh in Threshold){
    ...
    MRres$b <- res.egger$b
    MRres$se <- res.egger$se
    MRres$pval <- res.egger$pval
...
for(i in 1:nrow(pairs)){
  ...
  if(nrow(dat) < 4) next
  MRres = run_egger(dat, Threshold=c(5e-05,5e-06,5e-07,5e-08),
                    exposure=trait1.name,outcome=trait2.name)

  write.table(MRres, "./res/Egger.MRres",
