...
for( exposure in ts1){
  for( outcome in ts2){
    ...
    trait2 = readr::read_delim(trait2.dir, "\t", escape_double = FALSE, trim_ws = TRUE, progress = F)
    ...
    write.table(matrix(as.vector(paras$Omega), nrow=1),
                paste0("./pairs_bg_paras/", exposure, "~", outcome, "_Omega"),
                row.names = F, col.names = F, quote = F)
    write.table(matrix(as.vector(paras$C), nrow=1),
                paste0("./pairs_bg_paras/", exposure, "~", outcome, "_C"),
                row.names = F, col.names = F, quote = F)
